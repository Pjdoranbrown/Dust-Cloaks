function openRoster() {
    ...
    META.roster.forEach((char, index) => {
        ...
        card.innerHTML = `<img src="${ASSETS[def.iconKey || def.imgKey]?.src}" class="card-icon"><div class="card-title" style="color:${def.color}">${char.name}</div><div class="card-role">${race.name} ${def.name}</div><div class="card-desc">Kills: ${char.kills||0}<br>Earned: ${char.gp||0}g</div>`;
